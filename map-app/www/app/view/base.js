define(["d3"], function(d3) {
	...
	base.prototype.makeAccordionAtD3Selection = function(opts) {
		...
		const accordion = opts.selection.append('div')
		...
		// is shown or hidden.
		const contentWrapper = opts.selection.append('div').classed(opts.hideContent ? "w3-hide" : "w3-show", true);
		function setAccordionTitle(wrapper) {
			accordion.attr('title', "Click to " +
		...
		accordion.on('click', function(e) {
			...
			if (contentWrapper.classed('w3-hide')) {
				...
				console.log("Unexpectedly found neither w3-hide nor w3-show classes in wrapper div for accordion contentWrapper");
			}
			setAccordionTitle(contentWrapper);
		})
		;
