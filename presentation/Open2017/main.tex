...
\frame[fragile,t]
{
  ...
  \centering
  %\begin{center}
  %\begin{tikzpicture}[overlay, scale=.8, show background rectangle, node distance=1cm]
  \begin{tikzpicture}[remember picture, overlay, show background rectangle, node distance=1cm]
    ...
    %\node[obj node,anchor=north] at (current page.north) (iseobj) {\underline{Data links to other datasets} e.g. \\ {\visible<2->{http://os.gov/postcode/OX10AB}} \\ {\visible<3->{http://ch.gov/company/12345678}}};
    ...
    \node[text width=1cm, visible on=<1->] (datalabel) [left = of iseobj] {Example \\ data:};
    \node[data node] (isedata) [below = of iseobj] {Co-op data};
    \node[data node, visible on=<2->] (osdata) [left = of isedata] {OS data};
    \node[data node, visible on=<3->] (chdata) [right = of isedata] {CH data};
    %\node[label node, visible on=<2->] (datasetlabel) [left = of osdata] {Datasets:};
    \node[text width=1.5cm, visible on=<1->] (datasetlabel) [left = of osdata] {Datasets:};
    ...
    \node[app node, visible on=<4->] (mapapp) [below left = of isedata] {Map};
    \node[app node, visible on=<5->] (nearestapp) [below right = of isedata] {Nearest};
    \node[text width=2.5cm, visible on=<4->] (appslabel) [left = of mapapp] {Apps:};
    ...
    \draw[<->, visible on=<4>] (mapapp) -- (osdata) node[midway, left]{query};
