...
module SeOpenData
  module CSV
    def CSV.convert(output_io, output_header_symbols, output_header_text, input_io, csv_row_reader, csv_opts)
      ...
      csv_out << output_header_symbols.map {|h| output_header_text[h] }
      csv_in.each do |row|
	begin
	  ...
	  csv_out << output_header_symbols.map {|h| r.send(h) }
	  #initiative = Hash[output_header_symbols.map {|h| [h, r.send(h)] }]
	  #pp initiative
